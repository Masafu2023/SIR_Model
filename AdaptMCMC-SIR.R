...
birth_rate <- 0.01  # Birth rate for youngest group(children)
death_rate <- 0.01  # Death rate 
vax_coverage <- 0.6  # 60% vaccination coverage at birth
vax_efficacy <- 0.8  # 80% vaccine efficacy (all-or-nothing immunity)
...
summary(mcmc_data_long)

# Remove rows with NA
# mcmc_data_long <- na.omit(mcmc_data_long)

# Use the cleaned data in ggplot
# ggplot(cleaned_data, aes(x = x_var, y = y_var)) + geom_line()
